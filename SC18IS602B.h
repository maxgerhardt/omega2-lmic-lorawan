...
enum SC18IS601B_SPI_BitOrder {
	SC18IS601B_SPI_LSBFIRST = 1,
	SC18IS601B_SPI_MSBFIRST = 0
};
...
public:
    ...
    SC18IS602B() : SC18IS602B(-1, -1, 0, 0, 0) {};

    /* if resetPin = -1 it will be ignored */
    SC18IS602B(int interruptPin, int resetPin, bool a0, bool a1, bool a2);
    ...
    bool configureSPI(
    		SC18IS601B_SPI_BitOrder lsbFirst,
    		SC18IS601B_SPI_Mode spiMode,
...
private:
    ...
    int interruptPin = -1;	/* interrupt pin */
    bool intWasConfigured = false;	/* helper var to remember configuration */
